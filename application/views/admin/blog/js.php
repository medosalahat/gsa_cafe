...
<script type="text/javascript">
    ...
    function <?=$tbl->get_status()?>_change(value, row) {
    ...
    function  <?= $tbl->get_text() ?>_formatter(value) {
        return '<p style="  overflow: auto;height: 60px;display: block;">' + value + '</p>';
    }

    function  <?= $tbl->get_title() ?>_formatter(value) {
        return '<p style="  overflow: auto;height: 60px;display: block;">' + value + '</p>';
    }

    function status(id, value) {
        $.post('<?= site_url('admin/home/change_blog_status')?>', {
            id: <?=$tbl->get_id()?>,
            status: value
        }, function (result) {
            var $table = $('#table');
    ...
    function main_page(id, value) {
        $.post('<?= site_url('admin/home/change_blog_main_page')?>', {
            id: <?=$tbl->get_id()?>,
            status: value
        }, function (result) {
            var $table = $('#table');
...
<script type="text/javascript">
    ...
    window.<?=$tbl->get_table()?>_events = {
        'click .remove': function (e, value, row, index) {
            ...
            if (confirms) {
                $.post('<?= site_url('admin/home/delete_blog')?>',
...
<script type="text/javascript">
    $('#form_edit_blog').bootstrapValidator({
...
<script type="text/javascript">
    var url = '<?= site_url('admin/home/change_image_blog')?>';
...
<script type="text/javascript">
    ...
    $(document).ready(function(){
        $('#add_blog_btn').click(function(){$('#add_row').modal('show');});
        $('#form_add_blog').bootstrapValidator({
            message: 'This value is not valid', feedbackIcons: {
                valid: 'glyphicon glyphicon-ok',
                invalid: 'glyphicon glyphicon-remove',
                validating: 'glyphicon glyphicon-refresh'
